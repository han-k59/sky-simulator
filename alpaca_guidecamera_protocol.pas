...
var
  ...
  pixel_size_guide_alpaca:double=5;//pixel size in micrometer
...
uses sky_annotation;{for img_array, utc_date_time and update_required}
...
begin
  result:=4; // ICamera version implementation
end;
...
function  T_Alpaca_Guidecam.pixelsizex: double;
begin
  result:=pixel_size_guide_alpaca;//pixel size in micrometer
end;

function  T_Alpaca_Guidecam.pixelsizey: double;
begin
  result:=pixel_size_guide_alpaca;//pixel size in micrometer
end;
...
begin
  result:=((img_array<>nil) and (last_exposureduration>=0) and (Percent_Completed>=100) and (update_required=false)); {image in the buffer and startexposure was given, image is still valid=(update_required=false)}
end;
...
function  T_Alpaca_Guidecam.DeviceState:TStringList;
...
begin
  result:=TStringList.Create;
  result.Clear;
    result.add('{"Name":"CameraState","Value":'+inttostr(CameraState)+'}');
    result.add('{"Name":"CCDTemperature","Value":'+floattostrFdot(CCDTemperature,0,1)+'}');
    result.add('{"Name":"CoolerPower","Value":'+floattostrFdot(CoolerPower,0,1)+'}');
    result.add('{"Name":"HeatSinkTemperature","Value":'+floattostrFdot(HeatSinkTemperature,0,1)+'}');
    result.add('{"Name":"ImageReady","Value":'+BoolToStr(ImageReady,'true','false')+'}');
    result.add('{"Name":"IsPulseGuiding","Value":'+BoolToStr(IsPulseGuiding,'true','false')+'}');
    result.add('{"Name":"PercentCompleted","Value":'+inttostr(PercentCompleted)+'}');
    result.add('{"Name":"TimeStamp","Value":"'+last_exposureStartTime+'"}');//string between  " "
end;
